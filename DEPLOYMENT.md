...
| `ANTHROPIC_API_KEY` | Required for the four Claude-powered tools (`lib/ai/*`): Message & Concept Testing (content/audience-fit), Competitive Scan (cross-site synthesis), Wireframe Analysis, and Design & Content Audit. No fallback exists — without this set, those four runs fail immediately with a clear error message persisted on the run (`ANTHROPIC_API_KEY is not configured…`); every other tool in the product works without it. |
...
5. With `ANTHROPIC_API_KEY` set, run one of each Claude-powered tool
   (Message & Concept Testing, Competitive Scan, Wireframe Analysis, Design &
